...
**Why discrete grades, and no combination step.** Under well-modeled conditions, with honest operators and demonstrably independent sources, a coherent probabilistic combination of source reliabilities would use more of the information than any profile of fragments. This specification refuses the combination step anyway, because it is written for the environment CRAFT declares: adversarial actors, unknown correlation between sources, and operators with an interest in the outcome selecting the weights. A single combined number is easy to tune and hard to audit; a profile of separately based fragments, exposed per dimension, is hard to tune silently and cheap to audit. The refusal to combine is not a limitation of the grading. It is the grading's defense.
...
**Support, defined.** An output's support is the transitive closure of the material it rests on: the sources it draws on directly, and the sources of every intermediate result it draws on, traced back to ingested origins. A source is material to an output when removing it, and everything only it supports, would change what the output can claim.

- Every output that rests on ingested material (a finding, an attestation, a published entry, a funding decision) MUST expose the grade profile of its material support: for each grading dimension, the weakest assessment present anywhere in that support, together with every flag carried by any source in it. The profile is per-dimension; no ordering across dimensions is implied, which is what Section 3's no-substitution requirement permits and requires.
