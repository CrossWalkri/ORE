...
**Copy-and-run prompts for grading and admitting sources at the ingestion boundary** · v0.2.0 · 2026-08-03 · CC0 1.0

Three prompts implementing ORE in any AI assistant. Each is self-contained: it carries the definitions it needs, so it works pasted into a fresh conversation with no prior context and no tooling. Paste one, add your material where marked, run it.
...
2. State the downstream condition each posture obliges, concretely: what must
   hold in my system for the posture to be real.
...
4. If any part is Open, describe how the quarantine boundary holds
   by structure rather than by policy, and name the residual channel: a quarantined
...
6. Give me the standing-loss process: how a source's eligibility can be reduced or
   ...
   finding about the source, and not applied retroactively to outputs already produced.
...
The specification: [ORE](ore-specification-0_1_2.md) for source grading, CC0. A test case with known ground truth is at [ore-benchmark-case-0_1_0.md](ore-benchmark-case-0_1_0.md). The output boundary, producing and auditing evidence-grade outputs, is its companion [STRUCK](https://github.com/CrossWalkri/STRUCK), which carries its own prompts.
